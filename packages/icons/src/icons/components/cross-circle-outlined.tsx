import { Icon as SvgIcon } from './base/svg-icon';
import { Icon as FontIcon } from './base/font-icon';
import svgSource from '../../svg/cross-circle-outlined.svg';

export type CrossCircleOutlinedProps = Omit<
  React.ComponentProps<typeof SvgIcon> & React.ComponentProps<typeof FontIcon>,
  'type'
>;
/**
 * ![cross-circle-outlined.svg](data:image/svg+xml;base64,)
 *
 * ---
 *
 * 支持最多配置 2 个颜色，按顺序分别为
 * ![#fff](data:image/svg+xml;base64,)
 * ![#f00](data:image/svg+xml;base64,)
 */
export const CrossCircleOutlined: React.FC<CrossCircleOutlinedProps> = (props) => {
  if (props.color) {
    return <SvgIcon type={svgSource} {...props} />;
  }
  return <FontIcon type='heathen-icon-cross-circle-outlined' {...props} />;
};
