import type { ColorInput, HSL, HSV, OptionalA, RGB, ColorToken } from './type';
...
const parseHSVorHSL: ParseNumber = (num, _, index) => (index === 0 ? num : num / 100);
...
export class FastColor {
  ...
  constructor(input: ColorInput) {
    ...
    function matchFormat(str: string) {
      return str[0] in (input as object) && str[1] in (input as object) && str[2] in (input as object);
    }
    ...
    if (!input) {
      ...
      throw new Error('[FastColor]: unsupported input ' + JSON.stringify(input));
    }
  }
  ...
  mix(input: ColorInput, amount = 50) {
    ...
    const p = amount / 100;
    const calc = (key: ColorToken) => (color[key] - this[key]) * p + this[key];
  ...
  private _sc(rgb: ColorToken, value: number, max?: number) {
